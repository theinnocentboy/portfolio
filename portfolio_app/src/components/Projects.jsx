  ...
  return (
    <section className="py-20 px-6 max-w-7xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        <h2 className="text-4xl md:text-5xl font-bold text-tokyonight-fg mb-4">Featured <span className="text-tokyonight-green">Architecture</span></h2>
        <p className="text-tokyonight-fgMuted mb-12 max-w-2xl text-lg">End-to-end software solutions highlighting AI integration, scalable backends, and responsive UI design.</p>
      </motion.div>
        ...
        {projects.map((project, index) => (
          <motion.div key={index} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }} whileHover={{ y: -10 }} className="bg-tokyonight-bgStorm border border-tokyonight-bgHighlight rounded-2xl p-8 shadow-xl hover:shadow-tokyonight-green/10 transition-all group relative overflow-hidden flex flex-col">
            ...
            <div className="relative z-10 flex-1">
              <h3 className="text-2xl font-bold text-tokyonight-fg mb-3 group-hover:text-tokyonight-green transition-colors">{project.title}</h3>
              <p className="text-tokyonight-fgMuted mb-6 leading-relaxed">{project.description}</p>
              
              <div className="flex flex-wrap gap-2 mb-8">
                {project.tech.map((tech, i) => (
                  <span key={i} className="px-3 py-1 bg-tokyonight-bgHighlight text-tokyonight-teal text-sm font-medium rounded-full">{tech}</span>
                ))}
              </div>
            </div>
            ...
            <div className="relative z-10 flex items-center gap-4 pt-4 border-t border-tokyonight-bgHighlight mt-auto">
              {project.live && (
                <a href={project.live} target="_blank" rel="noreferrer" className="bg-tokyonight-green/90 hover:bg-tokyonight-green text-tokyonight-bg px-4 py-2 rounded-lg flex items-center gap-2 transition-colors font-semibold text-sm shadow-[0_0_15px_rgba(158,206,106,0.2)] hover:shadow-[0_0_25px_rgba(158,206,106,0.4)]">
                  <ExternalLink size={16} /> Live Demo
                </a>
              )}
              <a href={project.link} target="_blank" rel="noreferrer" className="text-tokyonight-fgDim hover:text-tokyonight-fg flex items-center gap-2 transition-colors text-sm font-medium px-4 py-2 bg-tokyonight-bgHighlight/60 hover:bg-tokyonight-bgHighlight rounded-lg">
